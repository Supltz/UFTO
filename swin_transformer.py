...
model_urls = {
     'swin_transformer_tiny': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_tiny_patch4_window7_224.pth',
     'swin_transformer_small': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_small_patch4_window7_224.pth',
     'swin_transformer_base': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window7_224.pth',
}
...
def swin_transformer_tiny(pretrained=True, **kwargs):
    ...
    if pretrained:
        if pretrained:
            state_dict = torch.load(os.path.join(models_dir, model_name['swin_transformer_tiny']))
            state_dict = {k: v for k, v in state_dict['model'].items() if not k.endswith(('head.weight', 'head.bias'))}
...
def swin_transformer_small(pretrained=True, **kwargs):
    ...
    if pretrained:
        if pretrained:
            state_dict = torch.load(os.path.join(models_dir, model_name['swin_transformer_small']))
            state_dict = {k: v for k, v in state_dict['model'].items() if not k.endswith(('head.weight', 'head.bias'))}
            model.load_state_dict(state_dict, strict=False)
    return model
...
def swin_transformer_base(pretrained=True, **kwargs):
    ...
    if pretrained:
        state_dict = torch.load(os.path.join(models_dir, model_name['swin_transformer_base']))
        state_dict = {k: v for k, v in state_dict['model'].items() if not k.endswith(('head.weight','head.bias'))}
        model.load_state_dict(state_dict, strict=False)
    return model
